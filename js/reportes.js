...
// ============================================================
// PANEL 3 — REPORTE DE VENTAS (rango de fechas)
// ============================================================
let reporteVentasActual = null;

async function generarReporteVentas() {
  const desde = document.getElementById("rv-desde")?.value;
  const hasta = document.getElementById("rv-hasta")?.value;
  const vendedor = document.getElementById("rv-vendedor")?.value.trim() || "";
  const metodo = document.getElementById("rv-metodo")?.value || "";

  const statusEl = document.getElementById("status-rv");
  if (statusEl) { statusEl.textContent = "Cargando…"; statusEl.classList.remove("error"); }

  let query = `${SUPABASE_URL}/rest/v1/facturas?select=*&order=${COL_FECHA}.asc`;

  if (desde) query += `&${COL_FECHA}=gte.${desde}T00:00:00`;
  if (hasta) {
    const d = new Date(hasta + "T00:00:00");
    d.setDate(d.getDate() + 1);
    query += `&${COL_FECHA}=lt.${d.toISOString().slice(0, 19)}`;
  }
  if (vendedor) query += `&vendedor=ilike.*${encodeURIComponent(vendedor)}*`;
  if (metodo) query += `&metodo_pago=eq.${encodeURIComponent(metodo)}`;
  ...
  try {
    const res = await fetch(query, { headers });
    if (!res.ok) throw new Error("Error " + res.status + " al consultar ventas");
    const facturas = await res.json();
    if (statusEl) statusEl.textContent = `Actualizado ${new Date().toLocaleTimeString("es-VE")}`;
    reporteVentasActual = { desde, hasta, facturas };
    renderReporteVentas(reporteVentasActual);
  } catch (err) {
    if (statusEl) { statusEl.textContent = "No se pudo cargar: " + err.message; statusEl.classList.add("error"); }
    reporteVentasActual = { desde, hasta, facturas: [] };
    renderReporteVentas(reporteVentasActual);
...
function renderReporteVentas(rep) {
  const facturas = rep.facturas;
  let totalUSD = 0, totalBs = 0, totalDescuento = 0;
  facturas.forEach((f) => {
    totalUSD += Number(f.total_usd) || 0;
    totalBs += Number(f.total_bs) || 0;
    totalDescuento += Number(f.descuento_usd) || 0;
  });
  const promedio = facturas.length ? totalUSD / facturas.length : 0;

  const kpiUsd = document.getElementById("rv-kpi-usd");
  const kpiBs = document.getElementById("rv-kpi-bs");
  const kpiCount = document.getElementById("rv-kpi-count");
  const kpiAvg = document.getElementById("rv-kpi-avg");
  const kpiDesc = document.getElementById("rv-kpi-desc");
  if (kpiUsd) kpiUsd.textContent = fmtUSD(totalUSD);
  if (kpiBs) kpiBs.textContent = fmtBS(totalBs);
  if (kpiCount) kpiCount.textContent = facturas.length;
  if (kpiAvg) kpiAvg.textContent = fmtUSD(promedio);
  if (kpiDesc) kpiDesc.textContent = fmtUSD(totalDescuento);

  const countLabel = document.getElementById("rv-count-label");
  if (countLabel) countLabel.textContent = `(${facturas.length} registros)`;

  const tbody = document.getElementById("tbody-rv");
  const empty = document.getElementById("empty-rv");
  if (tbody) tbody.innerHTML = "";
  if (!facturas.length) {
    if (empty) empty.style.display = "block";
  } else {
...
function imprimirReporteVentas() {
  ...
  const { desde, hasta, facturas } = reporteVentasActual;
  ...
  const filas = facturas.map((f) => {
    ...
    return `<tr>
      ...
      <td>${escapeHtml(f.nombre || "")} ${escapeHtml(f.apellido || "")}</td>
      <td>${escapeHtml(f.vendedor || "")}</td>
      <td>${escapeHtml(nombreMetodo(f.metodo_pago))}</td>
      <td class="num">$${subtotal.toFixed(2)}</td>
      <td class="num">$${(Number(f.descuento_usd) || 0).toFixed(2)}</td>
      <td class="num">$${(Number(f.total_usd) || 0).toFixed(2)}</td>
      <td class="num">Bs ${(Number(f.total_bs) || 0).toFixed(2)}</td>
    </tr>`;
  }).join("");

  const periodo = (desde || hasta)
    ? `Periodo: ${desde ? escapeHtml(desde) : "…"} al ${hasta ? escapeHtml(hasta) : "…"}`
    : "Periodo: todas las fechas disponibles";

  const contenido = `
    <div class="rep-kpis">
      <div class="rep-kpi"><div class="label">Facturas</div><div class="value">${facturas.length}</div></div>
      <div class="rep-kpi"><div class="label">Total USD</div><div class="value">$${totalUSD.toFixed(2)}</div></div>
      <div class="rep-kpi"><div class="label">Total Bs</div><div class="value">Bs ${totalBs.toFixed(2)}</div></div>
      <div class="rep-kpi"><div class="label">Descuentos</div><div class="value">$${totalDescuento.toFixed(2)}</div></div>
    </div>
    <table class="rep-tabla">
      <thead>
        <tr>
          <th>ID</th><th>Fecha</th><th>Cliente</th><th>Vendedor</th><th>Método</th>
          <th class="num">Subtotal</th><th class="num">Desc.</th><th class="num">Total $</th><th class="num">Total Bs</th>
        </tr>
      </thead>
      <tbody>${filas || '<tr><td colspan="9">Sin facturas en el periodo seleccionado.</td></tr>'}</tbody>
      <tfoot>
        <tr>
          <td colspan="7">TOTALES</td>
          <td class="num">$${totalUSD.toFixed(2)}</td>
          <td class="num">Bs ${totalBs.toFixed(2)}</td>
        </tr>
      </tfoot>
    </table>`;

  imprimirDocumento(plantillaA4("Reporte de ventas", periodo, contenido));
}

function exportarCSVVentas() {
  if (!reporteVentasActual) return;
  const filas = [["ID", "Fecha", "Cliente", "Cédula", "Vendedor", "Método", "Subtotal USD", "Descuento USD", "Total USD", "Total Bs"]];

  reporteVentasActual.facturas.forEach((f) => {
    const subtotal = (Number(f.total_usd) || 0) + (Number(f.descuento_usd) || 0);
    filas.push([
      f.id_factura || "",
      fmtFecha(f[COL_FECHA]),
      `${f.nombre || ""} ${f.apellido || ""}`.trim(),
      f.cedula || "",
      f.vendedor || "",
      nombreMetodo(f.metodo_pago),
      subtotal.toFixed(2),
      (Number(f.descuento_usd) || 0).toFixed(2),
      (Number(f.total_usd) || 0).toFixed(2),
      (Number(f.total_bs) || 0).toFixed(2),
    ]);
  });
  ...
  const rango = (reporteVentasActual.desde || reporteVentasActual.hasta)
    ? `${reporteVentasActual.desde || "inicio"}_a_${reporteVentasActual.hasta || "hoy"}`
    : hoyLocalISO();
  a.href = url;
  a.download = `reporte-ventas_${rango}.csv`;
...
let reporteComisionesActual = null;
...
async function generarReporteComisiones() {
  const mesSeleccionado = document.getElementById("rc-mes")?.value;
  const statusEl = document.getElementById("status-rc");

  if (!mesSeleccionado) { if (statusEl) statusEl.textContent = "Selecciona un mes."; return; }
  if (statusEl) { statusEl.textContent = "Calculando…"; statusEl.classList.remove("error"); }
  ...
  try {
    ...
    const mapaPagos = {};
    pagosData.forEach((p) => { if (p.vendedor) mapaPagos[p.vendedor.trim()] = p.pagado; });

    const mapaConfig = {};
    configData.forEach((c) => { if (c.vendedor) mapaConfig[c.vendedor.trim()] = c; });
    ...
    facturasData.forEach((f) => {
      ...
      if (!porVendedor[v]) {
        ...
        porVendedor[v] = { ventas: 0, total: 0, porcentaje, pagado: mapaPagos[v] !== undefined ? mapaPagos[v] : false };
      }
      porVendedor[v].ventas += 1;
      porVendedor[v].total += Number(f.total_usd) || 0;
    });

    if (statusEl) statusEl.textContent = `Comisiones de ${mesVentas} · Actualizado ${new Date().toLocaleTimeString("es-VE")}`;
    reporteComisionesActual = { mesSeleccionado, mesVentas, porVendedor };
    renderReporteComisiones(reporteComisionesActual);
  } catch (err) {
    console.error(err);
    if (statusEl) { statusEl.textContent = "No se pudo calcular: " + err.message; statusEl.classList.add("error"); }
  }
}

function renderReporteComisiones(rep) {
  const { porVendedor, mesVentas } = rep;
  const periodoLabel = document.getElementById("rc-periodo-label");
  if (periodoLabel) periodoLabel.textContent = `Ventas del mes de ${mesVentas}`;

  const tbody = document.getElementById("tbody-rc");
  const empty = document.getElementById("empty-rc");
  if (tbody) tbody.innerHTML = "";
  ...
  if (!vendedores.length) {
    ...
    vendedores
      ...
      .forEach((v) => {
        ...
        if (!datos.pagado) pendiente += comision;
        ...
        tr.innerHTML = `
          ...
          <td><span class="tag ${datos.pagado ? "" : "pend"}">${datos.pagado ? "Pagada" : "Pendiente"}</span></td>`;
        tbody.appendChild(tr);
      });
  }

  const kpiTotal = document.getElementById("rc-kpi-total");
  const kpiVend = document.getElementById("rc-kpi-vend");
  const kpiPend = document.getElementById("rc-kpi-pend");
  if (kpiTotal) kpiTotal.textContent = fmtUSD(totalComisiones);
  if (kpiVend) kpiVend.textContent = vendedores.length;
  if (kpiPend) kpiPend.textContent = fmtUSD(pendiente);
}

function imprimirReporteComisiones() {
  if (!reporteComisionesActual) return;
  const { porVendedor, mesSeleccionado, mesVentas } = reporteComisionesActual;
  const vendedores = Object.keys(porVendedor).sort((a, b) => porVendedor[b].total - porVendedor[a].total);
  ...
  const filas = vendedores.map((v) => {
    ...
    return `<tr>
      <td>${escapeHtml(v)}</td>
      <td class="num">${datos.ventas}</td>
      <td class="num">$${datos.total.toFixed(2)}</td>
      <td class="num">${(datos.porcentaje * 100).toFixed(1)}%</td>
      <td class="num">$${comision.toFixed(2)}</td>
      <td>${datos.pagado ? "Pagada" : "Pendiente"}</td>
    </tr>`;
  }).join("");

  const contenido = `
    <div class="rep-kpis">
      <div class="rep-kpi"><div class="label">Vendedores</div><div class="value">${vendedores.length}</div></div>
      <div class="rep-kpi"><div class="label">Comisiones</div><div class="value">$${totalComisiones.toFixed(2)}</div></div>
      <div class="rep-kpi"><div class="label">Pendiente</div><div class="value">$${pendiente.toFixed(2)}</div></div>
    </div>
    <table class="rep-tabla">
      <thead>
        <tr><th>Vendedor</th><th class="num"># Ventas</th><th class="num">Total vendido</th><th class="num">%</th><th class="num">Comisión</th><th>Estado</th></tr>
      </thead>
      <tbody>${filas || '<tr><td colspan="6">Sin ventas registradas en el mes.</td></tr>'}</tbody>
      <tfoot>
        <tr><td colspan="4">TOTALES</td><td class="num">$${totalComisiones.toFixed(2)}</td><td></td></tr>
      </tfoot>
    </table>`;

  imprimirDocumento(plantillaA4(
    "Reporte de comisiones",
    `Mes de pago: ${mesSeleccionado} · Ventas de: ${mesVentas}`,
    contenido
  ));
}
